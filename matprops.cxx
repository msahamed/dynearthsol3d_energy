  ...
  elemmarkers(*var.elemmarkers)
{
    ...
    thermal_coefficient = VectorBase::create(p.mat.thermal_coefficient, nmat);
...
double MatProps::alpha(int e) const
{
    return harmonic_mean(*thermal_coefficient, elemmarkers[e]);
}
...
double MatProps::rho(int e) const
{
    ...
    for (int m=0; m<nmat; m++) {
        ...
        result += (*rho0)[m] * (1 - (*thermal_coefficient)[m] * TinCelsius) * elemmarkers[e][m];
        n += elemmarkers[e][m];
    }
    return result / n;
}
